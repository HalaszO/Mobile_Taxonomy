...
file_filter='g*.dat';    % files to be included in analysis
...
for cell=1:ncells,
    ...
    t=1e-3*datin(1:4500,1);
    vm=1e-3*datin(1:4500,2:end);
    ...
    subplot(2,2,3)
    set(gcf,'DefaultAxesColorOrder',[1 0 0;0 1 0])
    plot(t(400:end),vm(400:end,:),'.')
    hold on
    for i=1:5,
        vi(cell,i)=exp1(t(501),bestparse{cell}(1+i),bestparse{cell}(1),bestparse{cell}(6+i));
        plot(t(501:end),exp1(t(501:end)-t(501),bestparse{cell}(1+i),bestparse{cell}(1),bestparse{cell}(6+i)),'k','LineWidth',2)
    end
