...
file_filter='s*.txt';    % files to be included in analysis
sampling_rate=5000;      % sampling frequency (in Hz)
cstep_all=-10:-10:-100;  % sequence of current step amplitudes (in pA)
step_start=0.2;
step_end=1.0;
protocol_indices = 2:2:20; % the indices of negative current injections
t_all = (0:0.2:1399.8)';

path_to_data=input('Path to data files (RETURN - current directory): ','s');
if strcmp(path_to_data,'')
    path_to_data='.';
elseif ~exist(path_to_data,'dir')
    disp('No such directory.')
    return
end

files=dir(strcat(path_to_data,'/',file_filter));
ncells = length(files);

fid = fopen(strcat(path_to_data,'/cellnames.txt'),'wt');
...
for cell=1:ncells
    ...
    datin=importdata(strcat(path_to_data,'/',fname{cell}));
    
    t=1e-3*t_all(1:step_end*sampling_rate);
    %t=1e-3*datin(1:step_end*sampling_rate,1);
    vm=1e-3*datin(1:step_end*sampling_rate, protocol_indices);
    ...
    print(fh,'-djpeg',strcat(path_to_data,'/',cellname{cell}))
end
...
dlmwrite(strcat(path_to_data,'/celldata.dat'), celldata_out, 'delimiter','\t', 'precision',4)
