...
for sweep=1:iv.sweepnum; 
    ...
    if iv.current(sweep) < 0 % Searching for sag using 500Hz filtering
        ...
        % Using the sag_extractor
        sagstats = sag_extractor(iv, sweep);
        data.rin_new(sweep) = sagstats.rinc;
        data.tau_new(sweep) = sagstats.tau_out2;
        data.capacity_new(sweep) = sagstats.capacity;
        data.vsag_new(sweep) = sagstats.vsag;
        data.relsag_new(sweep) = sagstats.relsagamp;
        data.sagdelay_new(sweep) = sagstats.sagdelay;
